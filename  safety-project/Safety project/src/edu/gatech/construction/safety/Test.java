...
/**
 * This rule checks "Elevators for Egress" based on a basic program. Each
 * sub-module should be updated in detail later.
 * 
 * @author Jin Kook Lee
 */
...
public class Test extends DefaultRule {
	...
	public void check(Entity entity) {
		...
		if (buildingHeight < 31.0) {
			...
			System.out.println("This building has " + elevatorCount
					+ " elevators.");

			for (int i = 0; i < stories.length; ++i) {
				floorNet = 0.0;
				for (int j = 0; j < spaces.length; j++) {
					SBuildingStorey floor = (SBuildingStorey) spaces[j]
							.getContainer();
					if (compStr(floor.name.getStringValue().trim(),
							stories[i].name.getStringValue().trim())) {
	...
	/**
	 * TODO Upgrade required. This method calculates building height: Sum of all
	 * floor's height. Under-ground floors're not considered. Slopped first
	 * floor is not considered.
	 * 
	...
	public static boolean contStr(String s1, String s2) {
		boolean bool = false;
		// s1 will be a standard name, and s2 is IFC instance name for
		// determining what s2 means.
		// lower cases without dots & double spaces: e.g) U.S. Attorney -> us
		// attorney, U.S.M.S -> usms
		try {
	...
	public static boolean compStr(String s1, String s2) {
		...
		if (s1.trim().length() == 0 && s2.trim().length() == 0) {
			...
			// s1 will be a standard name, and s2 is IFC instance name for
			// determining what s2 means.
			// lower cases without dots & double spaces: e.g) U.S. Attorney ->
			// us attorney, U.S.M.S -> usms
			try {
